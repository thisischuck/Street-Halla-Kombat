...
namespace SHK
{
    ...
    public partial class TexturedPrimitive
    {
        ...
        {
            if (sTextureData.ContainsKey(ImageName))
                mTextureColor = sTextureData[ImageName];
            else
                mTextureColor = LoadColorInfo(ImageName, mImage);
        }
        ...
        {
            float x = i * mSize.X / (float)(SpriteImageWidth - 1);
            float y = j * mSize.Y / (float)(SpriteImageHeight- 1);
        ...
        {
            Vector2 delta = p - mPosition;
            float xOffset = Vector2.Dot(delta, xDir);
            float yOffset = Vector2.Dot(delta, yDir);
            float i = SpriteImageWidth * (xOffset / mSize.X);
            float j = SpriteImageHeight * (yOffset / mSize.Y);
